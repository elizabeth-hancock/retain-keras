...
from tensorflow.utils.data_utils import Sequence
from tensorflow.constraints import non_neg, Constraint
...
class SequenceBuilder(Sequence):
    ...
    def __init__(self, data, target, batch_size, ARGS, target_out=True):
        ...
        :param ARGS: Other anciliary parameters
        :type ARGS: dict
...
def model_create(ARGS):
    ...
    def retain(ARGS):
        ...
        The Keras Layer design is mimicing RETAIN architecture.
        :param ARGS: Arguments object containing user-specified parameters
        :type ARGS: :class:`argparse.Namespace`
        :return: Keras model
        :rtype: :class:`tensorflow.keras.Model`
        ...
        codes_embs = tf.reduce_sum(codes_embs_total, axis=2)
        ...
        contexts = L.Lambda(lambda x: tf.reshape(tensor=x,
            shape=(tf.shape(x)[0], 1, ARGS.emb_size + ARGS.numeric_size)))(c_t)
    ...
    # Check GPUs available + set up strategy for multi-gpu  processing
...
def create_callbacks(model, data, ARGS):
    """At the end of each epoch, determine various callback
    statistics (e.g. ROC-AUC)
    ...
    class LogEval(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            """
            Store and write various metrics for the end of an epoch
            :param int epoch: epoch number
            :param logs: dictionary to store various metrics, defaults to {}
    ...
    lr_reduce = ReduceLROnPlateau("val_loss", .25, 3, verbose=1)
    return(checkpoint, log, lr_reduce)


def train_model(model, data_train, y_train, data_test, y_test, ARGS):
    """
    Class to hold callback artificats, Sequence builder of training data, model training
    generator
    ...
    checkpoint, log, reduce_lr = create_callbacks(model, (data_test, y_test), ARGS)
    ...
    model.fit_generator(generator=train_generator, epochs=ARGS.epochs,
                        ...
                        callbacks=[checkpoint, log, reduce_lr], verbose=1, workers=3, initial_epoch=0)
...
def main(ARGS):
    ...
    end_time = time.time()
    time_elapsed = ( end_time - start_time )/ (60*60)
    print("RETAIN training time in hours: " , time_elapsed)
